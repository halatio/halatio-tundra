"""MS SQL Server connector using Polars + ConnectorX"""
...
class MSSQLConnector(BaseConnector):
    """Production-grade MS SQL Server connector"""
    ...
    def _build_connection_string(self) -> str:
        ...
        return (
            f"mssql://{self.credentials['username']}:{self.credentials['password']}"
            f"@{self.credentials['host']}:{self.credentials.get('port', 1433)}"
    ...
    async def extract_to_parquet(
        ...
        **kwargs
        ...
        """
        Extract MS SQL Server data to Parquet using ConnectorX
        ...
        try:
            ...
            logger.info(f"Extracting MS SQL Server data with query: {query[:100]}...")
            ...
            logger.error(f"MS SQL Server extraction failed: {str(e)}")
            raise
